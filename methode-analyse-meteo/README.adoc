...
NOTE: On considère qu'une bulle d'air chaud se détache du sol lorsque sa température est supérieure de 3° C à celle de l'air ambiant.
...
===== Cas d'une masse d'air instable
...
Cette masse d'air est donc instable.
...
[NOTE]
. Cas avec une couche d'inversion
======
Prenons la même situation à la différence prêt que la masse d'air s'arrête à 1500 m d'altitude.
Au-dessus d'elle se trouve une masse d'air de 200 m d'altitude et où la témpérature croît de 0,1° C par 100 m.
La température s'inverse dans cette masse d'air que l'on appelle alors une couche d'inversion.
Dans ce cas, l'air va s'élever jusqu'à atteindre cette couche.
======
