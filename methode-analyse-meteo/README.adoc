...
image::windy-depresssion-anticyclone.png[]
...
On peut voir sur Windy les zones du vent.
...
La zone d'Annecy semblent donc plutôt propice.
...
On regarde les nuages sur plusieurs jours.
...
On peut voir qu'Annecy est peu concerné par les nuages.

===== Étude des précipitations

On regarde les précipitations pour les jours à venir.
...
On peut voir qu'il n'y a pas de précipitations à prévoir à Annecy pour les jours à venir.
...
Il convient maintenant d'étudier plus finement.
...
On se rend sur le site www.meteoblue.com[Météo Blue], on tape le nom de la ville qui nous intéresse (ici Talloires).
...
En faisant la même analyse pour lundi, on peut en déduire que la journée de lundi est également propice aux vols thermiques (avec une apparation et une disparition de l'instabilité aux mêmes créneaux horaires que dimanches).
